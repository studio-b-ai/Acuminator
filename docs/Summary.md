...
| [PX1028](diagnostics/PX1028.md) | Constructors in DACs and DAC extensions are prohibited. | Error | Available |
...
| [PX1122](diagnostics/PX1122.md) | A `PXDatabase.Execute` call uses an `INSERT INTO RolesIn[Graph|Member|Cache] SELECT ... FROM RolesIn[...]` pattern that silently null-ops when the source `ScreenID` has zero rows. Add a pre-COUNT check on the source plus a `Rolename='*'` fallback. | Warning | Unavailable |
| [PX1123](diagnostics/PX1123.md) | A `PXDatabase.Execute` call performs an `INSERT INTO RolesInMember` or `INSERT INTO RolesInCache` with an explicit column list that omits `Cachetype`. Those tables require `Cachetype` (NOT NULL); the INSERT will crash at runtime. Add `Cachetype` to the column list or use `INFORMATION_SCHEMA.COLUMNS` for dynamic discovery. | Warning | Unavailable |
